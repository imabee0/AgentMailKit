...
mod tests {
    ...
    /// difference between "correctly gated" and "gated on a name nothing ever grants". This list
    /// is generated by `grep -rn 'permissions::require(&ctx.grants, "' crates/amk-http/src` and
    /// must be updated by the same command whenever a handler adds or renames a gate.
    #[test]
    fn every_permission_flag_literal_used_by_a_handler_is_a_real_flag() {
        let used_by_handlers = [
            "pod_read",
            "pod_create",
            "pod_delete",
            "api_key_read",
        ...
        for flag in used_by_handlers {
            assert!(
                amk_core::permissions::is_known_flag(flag),
                "{flag} is not a real ApiKeyPermissions flag — a handler is gating on a typo"
            );
        }
    }
}
